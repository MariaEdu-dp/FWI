...
for dataset in data:
    ...
    print(ano)
    mean = dataset.mean(dim="valid_time")
    print(mean)
    mean.to_netcdf(f"datasets/media_10/Media_{ano}.nc")

    std = dataset.std(dim="valid_time")
    print(std)
    std.to_netcdf(f"datasets/desvio_pad_10/Desvio_padrao_{ano}.nc")

    polyfit = dataset.polyfit(dim="time", deg=1) * 1e9 * 60 * 60 * 24 * len(dataset.valid_time.values)
    polyfit.to_metcdf(f"datasets/tendencia/tendencia_{ano}.nc")

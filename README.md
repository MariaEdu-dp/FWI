...
```ANOMALIA_MENSAL.py```: Cria anomalias mensais de FWI a partir da média mensal total de FWI e depois salva a anomalia média para cada década. Os dados são salvos na pasta ```datasets/ANOMALIAS/MENSAIS```. Vale lembrar que este código calcula as anomalias não baseadas na década, mas sim baseado em cada mês do período _total_. Depois, com as anomalias mensais calculadas, ele faz a média, por exemplo, de todos os meses de janeiro da primeira década, visando obter a anomalia mensal média para o mês de janeiro para cada intervalo em análise (ele faz isso para todos os meses; cada arquivo contém as anomalias mensais).

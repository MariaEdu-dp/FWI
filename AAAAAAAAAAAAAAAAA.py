...
for idx, clipped in enumerate(recortes):
    ...
    # Certifique-se de acessar os valores corretamente sem colchetes
    time_values = clipped["time"].values
    fwi_values = clipped.values

    x = mdates.date2num(time_values)

    z4 = np.polyfit(x, fwi_values, 3)
    p4 = np.poly1d(z4)

    # Plotar o FWI e a linha de tendência
    ax.plot(time_values, fwi_values, marker='o', label=f'{nomes[idx]}', linestyle='-')
    ax.plot(time_values, p4(time_values), linestyle='--', lw=0.8)
